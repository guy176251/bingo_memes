...
class UserCreateSerializer(serializers.Serializer):
    ...
    email = serializers.CharField(validators=[EmailValidator(), validators.email_unique])

    password = serializers.CharField(validators=[validators.password_length])
    ...
    def create(self, validated_data):
        ...
        site_user = SiteUser.objects.create(
            name=validated_data["username"], auth_user=user, score=0
        )
        return site_user
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(
        validators=[validators.only_letters_and_numbers, validators.length_is_(20)]
    )
    ...
    class Meta:
        ...
        extra_kwargs = {field: is_immutable for field in ["id"]}
    ...
    def get_related_categories(self, category: BingoCardCategory):
        ...
        related_names = category.subscription_set.values_list(
            "user__subscriptions__name", flat=True
        )
        if not related_names.exists():
            return []

        related_dict: dict[str, int] = {}
        ...
        return [
            CategoryRelatedSerializer(c, context={"request": request}).data
...
class CategorySmallSerializer(serializers.ModelSerializer):
    class Meta:
        model = BingoCardCategory
        fields = ["name"]
        extra_kwargs = {"name": {"validators": [validators.category_name_exists]}}
...
class CardListSerializer(CardDetailSerializer):
    ...
    def create(self, card_data):
        ...
        with atomic():
            ...
            Vote.objects.create(user=author, card=card, up=True)

            self.fields["tiles"].create(
                [{"text": tile["text"], "card": card} for tile in tile_data]
            )
...
class CategorySubscribeSerializer(serializers.ModelSerializer):
    id = serializers.IntegerField(validators=[validators.category_id_exists], read_only=True)
