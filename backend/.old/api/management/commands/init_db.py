...
from api.models import BingoCard  # UserSubscription,; CategorySubscription,
from api.models import BingoCardCategory, BingoTile, SiteUser, Vote
...
category_description = "This is one of several test categories that are used in the debug site. "
...
def create_dummy_data() -> DummyData:
    ...
    category_names = {t: {f"{t}_{c}" for c in {"A", "B", "C", "D", "E"}} for t in category_types}
        ...
        itertools.chain.from_iterable(
            itertools.chain.from_iterable(
                (
                    (
                        DummyCard(
                            name=f"{name}_CARD_{i} #"
                            + " #".join(sample(all_tag_texts, randint(1, 3))),
                            category_name=name,
...
def init_db_new():
    ...
    for usr in dummy_data.users:
        ...
        follow_pool = [follower for follower in dummy_data.users if follower.obj.id != usr.obj.id]
    ...
    Vote.objects.bulk_create(
        [Vote(user=card.obj.author, card=card.obj, up=True) for card in dummy_data.cards]
    )
    ...
    BingoCard.objects.bulk_update(
        [card.obj for card in dummy_data.cards], ["created_timestamp", "hot"]
    )
    ...
    for card in dummy_data.cards:
        voter_pool = [usr for usr in dummy_data.users if card.obj.author.id != usr.obj.id]

        for voter in sample(voter_pool, randint(1, 10)):
            Vote.objects.create(user=voter.obj, card=card.obj, up=True)
...
def init_db():
    ...
    for i in range(1, num_of_usrs):
        ...
        site_usr = SiteUser.objects.create(
            name=auth_usr.username, auth_user=auth_usr, score=num_of_cards
        )
    ...
    BingoCard.objects.bulk_create(
        [
            BingoCard(
                author=site_usrs[i // num_of_cards],
    ...
    for tile_texts, card in zip(tile_text_groups, cards):
        BingoTile.objects.bulk_create([BingoTile(text=text, card=card) for text in tile_texts])
    ...
    Vote.objects.bulk_create(
        [Vote(user=card.author, card=card, up=True) for index, card in enumerate(cards)]
    )
