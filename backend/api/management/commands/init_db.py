...
def create_dummy_data() -> DummyData:
    ...
    category_names = {
        t: {f"{t}_{c}" for c in {"A", "B", "C", "D", "E"}} for t in category_types
    }
...
def init_db_new():
    ...
    for usr in dummy_data.users:
        ...
        follow_pool = [
            follower for follower in dummy_data.users if follower.obj.id != usr.obj.id
        ]
    ...
    for card in dummy_data.cards:
        voter_pool = [
            usr for usr in dummy_data.users if card.obj.author.id != usr.obj.id
        ]
...
def init_db():
    ...
    for tile_texts, card in zip(tile_text_groups, cards):
        BingoTile.objects.bulk_create(
            [BingoTile(text=text, card=card) for text in tile_texts]
        )
